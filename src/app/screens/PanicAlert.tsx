...
import { triggerPanic, resolvePanic } from "../../services/panic.service";
import { useState, useEffect } from "react";
import { toast } from "sonner";
import type { PanicAlert as PanicAlertType } from "../../lib/database.types";
...
export function PanicAlert() {
  ...
  const [alert, setAlert] = useState<PanicAlertType | null>(null);
  ...
  const memberNames = members
    ...
    .map((m) => m.profiles?.full_name?.split(" ")[0] || "Miembro")
    .join(", ");

  // Get location then send panic
  useEffect(() => {
    if (!user || !sessionId) return;

    const send = (lat?: number, lng?: number) => {
      triggerPanic(sessionId, user.id, lat, lng)
        .then(setAlert)
  ...
  return (
    <div className="flex flex-col h-full bg-[#0A0A0F] overflow-y-auto no-scrollbar relative animate-in fade-in duration-300">
      ...
      <div className="px-6 py-12 flex-1 flex flex-col items-center relative z-10">
        ...
        <h1 className="text-[36px] font-black text-white mb-2 tracking-tighter uppercase drop-shadow-[0_4px_16px_rgba(255,59,48,0.5)]">
          Alerta enviada
        </h1>
        <p className="text-[#FF3B30] text-[16px] font-medium tracking-wide mb-10 text-center uppercase">
          Tu grupo está en camino
        </p>

        {/* Status cards */}
        <div className="w-full flex flex-col gap-3 mb-10">
          {[
            { Icon: MapPin, label: location ? `Lat ${location.lat.toFixed(4)}, Lng ${location.lng.toFixed(4)}` : "Ubicación compartida con tu grupo" },
            { Icon: Smartphone, label: memberNames ? `Notificación enviada a ${memberNames}` : "Notificando a tu grupo..." },
            { Icon: Users, label: "Contactos de emergencia alertados" },
          ].map(({ Icon, label }, i) => (
            <motion.div
              key={i}
              ...
              className="flex items-center gap-4 bg-[#14141C]/80 p-4 rounded-2xl border border-[#FF3B30]/30 shadow-[0_4px_24px_rgba(255,59,48,0.1)] backdrop-blur-md relative overflow-hidden group"
            >
              ...
              <span className="text-[15px] font-medium text-[#F0F0F5] leading-snug">{label}</span>
              <div className="ml-auto w-6 h-6 rounded-full bg-[#39FF6E]/10 border border-[#39FF6E]/30 flex items-center justify-center shrink-0">
        ...
        <div className="w-full bg-[#1E1E2A] rounded-2xl p-4 border border-[#2A2A38] shadow-inner relative overflow-hidden">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex flex-col justify-center">
              <span className="text-[15px] font-bold text-[#F0F0F5] mb-1">
                {session?.venue || "Ubicación de la salida"}
              </span>
              <span className="text-[13px] text-[#8888AA] font-mono tracking-wide">
                Última actualización: {timeStr}
              </span>
              <span className="text-[13px] font-bold text-[#F0F0F5] mt-1">
      ...
      <div className="px-6 pb-8 pt-4 shrink-0 bg-gradient-to-t from-[#0A0A0F] via-[#0A0A0F]/90 to-transparent sticky bottom-0 z-20">
        <Button
          ...
          disabled={cancelling}
          className="h-[64px] text-[18px] w-full border-[#F0F0F5]/50 text-[#F0F0F5] hover:bg-[#F0F0F5]/10 hover:border-[#F0F0F5] transition-all group shadow-[0_8px_32px_rgba(0,0,0,0.5)] bg-[#0A0A0F] disabled:opacity-60"
        >
          <X size={24} className="group-hover:rotate-90 transition-transform duration-300" strokeWidth={2.5} />
          {cancelling ? "Cancelando..." : "Estoy bien — Cancelar alerta"}
        </Button>
